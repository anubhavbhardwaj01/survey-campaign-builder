...
function StylingEditor({styles,open,toggle,patchStyle,survey,update}:{styles:Styles;open:Record<string,boolean>;toggle:(x:string)=>void;patchStyle:(p:Partial<Styles>)=>void;survey:Survey;update:(fn:(s:Survey)=>Survey)=>void}){
 ...
 return <div className="space-y-1">{section('appearance','Appearance',<div className="grid grid-cols-2 gap-4"><Color label="Background" value={styles.bg} set={v=>patchStyle({bg:v})}/><Color label="Backdrop" value={styles.backdrop} set={v=>patchStyle({backdrop:v})}/><Range label="Corner radius" value={styles.radius} min={0} max={48} set={v=>patchStyle({radius:v})}/><Range label="Backdrop opacity" value={styles.backdropOpacity} min={0} max={70} set={v=>patchStyle({backdropOpacity:v})}/></div>)}{section('typography','Question title',<Typography title="Title" color={styles.titleColor} setColor={v=>patchStyle({titleColor:v})} size={styles.titleSize} setSize={v=>patchStyle({titleSize:v})} weight={styles.titleWeight} setWeight={v=>patchStyle({titleWeight:v})} align={styles.titleAlign} setAlign={v=>patchStyle({titleAlign:v})} italic={styles.titleItalic} setItalic={v=>patchStyle({titleItalic:v})}/>)}{section('subtitle','Subtitle',<Typography title="Subtitle" color={styles.subColor} setColor={v=>patchStyle({subColor:v})} size={styles.subSize} setSize={v=>patchStyle({subSize:v})} weight={styles.subWeight} setWeight={v=>patchStyle({subWeight:v})} align={styles.subAlign} setAlign={v=>patchStyle({subAlign:v})}/>)}{section('options','Option list',<div className="space-y-4"><div className="grid grid-cols-2 gap-3"><Color label="Option background" value={styles.optionBg} set={v=>patchStyle({optionBg:v})}/><Color label="Option text" value={styles.optionText} set={v=>patchStyle({optionText:v})}/><Color label="Border" value={styles.optionBorder} set={v=>patchStyle({optionBorder:v})}/><Color label="Selected background" value={styles.optionSelectedBg} set={v=>patchStyle({optionSelectedBg:v})}/><Color label="Selected text" value={styles.optionSelectedText} set={v=>patchStyle({optionSelectedText:v})}/><Range label="Border width" value={styles.optionBorderWidth} min={0} max={4} set={v=>patchStyle({optionBorderWidth:v})}/><Range label="Height" value={styles.optionHeight} min={38} max={72} set={v=>patchStyle({optionHeight:v})}/><Range label="Spacing" value={styles.optionSpacing} min={4} max={24} set={v=>patchStyle({optionSpacing:v})}/><Range label="Corner radius" value={styles.optionRadius} min={0} max={28} set={v=>patchStyle({optionRadius:v})}/></div></div>)}{section('comments','Comment field',<div className="grid grid-cols-2 gap-3"><Color label="Background" value={styles.commentBg} set={v=>patchStyle({commentBg:v})}/><Color label="Text" value={styles.commentText} set={v=>patchStyle({commentText:v})}/><Color label="Border" value={styles.commentBorder} set={v=>patchStyle({commentBorder:v})}/><Range label="Border width" value={styles.commentBorderWidth} min={0} max={4} set={v=>patchStyle({commentBorderWidth:v})}/></div>)}{section('cta','CTA Button',<div className="grid grid-cols-2 gap-3"><Color label="Background" value={styles.ctaBg} set={v=>patchStyle({ctaBg:v})}/><Color label="Text" value={styles.ctaText} set={v=>patchStyle({ctaText:v})}/><Color label="Border" value={styles.ctaBorder} set={v=>patchStyle({ctaBorder:v})}/><Range label="Border width" value={styles.ctaBorderWidth} min={0} max={4} set={v=>patchStyle({ctaBorderWidth:v})}/><Range label="Height" value={styles.ctaHeight} min={38} max={72} set={v=>patchStyle({ctaHeight:v})}/><Range label="Radius" value={styles.ctaRadius} min={0} max={28} set={v=>patchStyle({ctaRadius:v})}/><Range label="Width %" value={styles.ctaWidth} min={30} max={100} set={v=>patchStyle({ctaWidth:v})}/><div className="col-span-2"><Toggle checked={styles.ctaFull} onChange={v=>patchStyle({ctaFull:v})} label="Full width"/></div></div>)}{section('close','Cross / Close button',<div className="grid grid-cols-2 gap-3"><Color label="Cross" value={styles.closeColor} set={v=>patchStyle({closeColor:v})}/><Color label="Fill" value={styles.closeFill} set={v=>patchStyle({closeFill:v})}/><Range label="Size" value={styles.closeSize} min={20} max={44} set={v=>patchStyle({closeSize:v})}/><div><label className="text-[11px] text-zinc-500">Style</label><select value={styles.closeStyle} onChange={e=>patchStyle({closeStyle:e.target.value as Styles['closeStyle']})} className="w-full mt-1 border rounded-lg p-2 text-xs bg-zinc-50"><option value="x">X</option><option value="minimal">Minimal X</option><option value="circle">Circle X</option><option value="rounded">Rounded X</option></select></div></div>)}{section('thank','Thank You styling',<div className="space-y-3"><p className="text-xs text-zinc-500">The Thank You page uses the same live canvas. Content controls are available under the Thank You section.</p><Color label="Title color" value={styles.titleColor} set={v=>patchStyle({titleColor:v})}/><Range label="Image width" value={80} min={30} max={100} set={()=>{}}/></div>)}</div>
}

function Typography({title,color,setColor,size,setSize,weight,setWeight,align,setAlign,italic=false,setItalic=()=>{}}:any){return <div className="space-y-4"><div className="grid grid-cols-2 gap-3"><Color label="Color" value={color} set={setColor}/><Range label="Size" value={size} min={10} max={48} set={setSize}/></div><div className="grid grid-cols-2 gap-3"><div><label className="text-[11px] text-zinc-500">Weight</label><select value={weight} onChange={e=>setWeight(+e.target.value)} className="w-full mt-1 border rounded-lg p-2 text-xs bg-zinc-50"><option value={400}>Regular</option><option value={500}>Medium</option><option value={600}>Semibold</option><option value={700}>Bold</option><option value={800}>Extra bold</option></select></div><div><label className="text-[11px] text-zinc-500">Alignment</label><select value={align} onChange={e=>setAlign(e.target.value)} className="w-full mt-1 border rounded-lg p-2 text-xs bg-zinc-50"><option>left</option><option>center</option><option>right</option></select></div></div><Toggle checked={italic} onChange={setItalic} label="Italic"/></div>}
